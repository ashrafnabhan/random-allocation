---
title: "Computer-generated Random allocation"
date: "`r Sys.time()`"
output: pdf_document
params:
  randlist: NA
...
This is the specifications of the randomization list "`r paste0("RandList ", Sys.Date()," seed ", params$seed, ".xslx")`" produced by a Shiny App. The application is based on R, and the code described in this document is also based on R.

# Parameters

The following parameters define the randomization list
...
`r if(is.null(ncol(params$stratalist))) {"There are no stratification variables."}` `r if(!is.null(ncol(params$stratalist))) {paste0("There are ", ncol(params$stratalist), " stratification variables")}` The stratification variable(s) parameter is given by:
